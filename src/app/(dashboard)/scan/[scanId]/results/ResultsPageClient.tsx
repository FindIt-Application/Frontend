	...
	const tabs = [
		{
			...
			label: "Subdomains",
			icon: Globe,
			count: mockResults.subdomains.length,
		},
		{
			id: "ports" as const,
			label: "Ports",
			icon: Server,
			count: mockResults.ports.length,
		},
		{
			id: "technologies" as const,
			label: "Technologies",
			icon: FileCode,
			count: mockResults.technologies.length,
		},
		{
			id: "dns" as const,
			label: "DNS Records",
			icon: Shield,
			count: mockResults.dns.length,
		},
	];
					...
					return (
						...
						>
							<div className="flex items-center gap-3">
								<div
									className={`p-2 rounded-lg ${activeTab === tab.id ? "bg-[#BD93F9]/30" : "bg-[#6272A4]/20"}`}
								>
									<Icon
										className={`w-5 h-5 ${activeTab === tab.id ? "text-[#BD93F9]" : "text-[#8BE9FD]"}`}
									/>
								</div>
								<div>
					...
					<table className="w-full">
						<thead>
							<tr className="border-b border-[#6272A4]/30">
								<th className="px-6 py-4 text-left text-sm font-medium text-[#6272A4]">
									Subdomain
								</th>
								<th className="px-6 py-4 text-left text-sm font-medium text-[#6272A4]">
									IP Address
						...
						<tbody>
							{mockResults.subdomains.map((sub, idx) => (
								...
								>
									<td className="px-6 py-4 text-[#50FA7B] font-mono text-sm">
										{sub.value}
									</td>
									<td className="px-6 py-4 text-[#F8F8F2] font-mono text-sm">
										{sub.ip}
									</td>
						...
						<tbody>
							{mockResults.ports.map((port, idx) => (
								<motion.tr
									...
									className="border-b border-[#6272A4]/20 hover:bg-[#6272A4]/10"
								>
									<td className="px-6 py-4 text-[#FFB86C] font-mono text-sm">
										{port.port}
									</td>
									<td className="px-6 py-4 text-[#F8F8F2] text-sm">
										{port.service}
									</td>
									<td className="px-6 py-4">
										<span
						...
						<tbody>
							{mockResults.technologies.map((tech, idx) => (
								<motion.tr
									...
									className="border-b border-[#6272A4]/20 hover:bg-[#6272A4]/10"
								>
									<td className="px-6 py-4 text-[#FF79C6] text-sm">
										{tech.name}
									</td>
									<td className="px-6 py-4 text-[#F8F8F2] font-mono text-sm">
										{tech.version}
									</td>
									<td className="px-6 py-4 text-[#6272A4] text-sm">
										{tech.category}
									</td>
								</motion.tr>
							))}
