...
export function parseAPIError(status: number, body: unknown): ClassifiedError {
	...
	const plainError = extractPlainErrorMessage(body);

	const baseError = {
		code: apiError?.code || `HTTP_${status}`,
		message: apiError?.message || plainError || getDefaultMessage(status),
...
function extractPlainErrorMessage(body: unknown): string | null {
	...
	if (
		...
		const nested = (obj.error as Record<string, unknown>).message as string;
		if (nested.trim() !== "") {
			return nested;
		}
	}
